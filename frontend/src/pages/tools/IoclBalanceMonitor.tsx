...
export default function IoclBalanceMonitor() {
  ...
  async function loadAll(initializeRecipients = true) {
    ...
    try {
      ...
      setMessage({ ok: false, text: visibleIssue(error, 'Could not load IOCL monitor settings.', isAdmin) })
    } finally {
      setLoading(false)
    }
  }
  ...
  useEffect(() => {
    if (!loading) void loadChecks().catch((error) => setMessage({ ok: false, text: visibleIssue(error, 'Could not load check history.', isAdmin) }))
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [checkPage, checkPageSize, checkStatus, checkTrigger])

  useEffect(() => {
    if (!loading) void loadNotifications().catch((error) => setMessage({ ok: false, text: visibleIssue(error, 'Could not load email history.', isAdmin) }))
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  async function saveSettings(panel: string, successText: string) {
    ...
    try {
      ...
      setMessage({ ok: false, text: visibleIssue(error, 'Could not save settings.', isAdmin) })
    } finally {
      setSavingPanel(null)
    }
  }
  ...
  async function testMail(mailType: 'daily' | 'alert') {
    ...
    try {
      ...
      setMessage({ ok: false, text: visibleIssue(error, 'Could not send the test mail.', isAdmin) })
    } finally {
      setTestingMail(null)
    }
  }
  ...
  async function checkNow() {
    ...
    try {
      ...
      setChecking(false)
      setMessage({ ok: false, text: visibleIssue(error, 'Could not start the balance check.', isAdmin) })
    }
  }
  ...
  async function uploadSession(file: File) {
    ...
    try {
      ...
      setMessage({ ok: false, text: visibleIssue(error, 'Could not import the browser session.', isAdmin) })
    } finally {
      ...
      <div className="flex flex-col gap-6">
        <GlassCard padding="lg" className="overflow-hidden">
          <div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-center">
            ...
            {/* Primary action stays at the top for this monitor/status page — intentional exception to the upload-tool bottom-placement convention. */}
        ...
        {message && (message.ok ? <p role="alert" className="rounded-xl border px-4 py-3 text-sm border-emerald-500/30 bg-emerald-500/10 text-emerald-600">{message.text}</p> : <ErrorBanner>{message.text}</ErrorBanner>)}
        {status.last_error && <ErrorBanner className="whitespace-pre-wrap break-words">{isAdmin ? 'Last check — technical details: ' : ''}{status.last_error}</ErrorBanner>}
