...
// ── non-blocking missing-mapping warnings ──────────────────────────────────
    ...
    <div className="flex flex-col gap-3 rounded-xl border border-amber-500/30 bg-amber-500/10 p-4">
      <div className="flex items-center gap-2 text-amber-600">
        ...
        <h4 className="font-display text-sm font-semibold">
          Report generated with unmapped values — the workbook was still written
        </h4>
      </div>
      <p className="text-sm text-ink-dim">
        Add these to the mapping tables below so future reports resolve them automatically.
      </p>
      {warnings.map((warning) => (
        <div key={warning.category} className="subpanel flex flex-col gap-2 p-3">
          <span className="text-xs font-bold tracking-[0.08em] text-amber-600 uppercase">
            {warning.category} ({formatIndianNumber(warning.items.length)})
          </span>
