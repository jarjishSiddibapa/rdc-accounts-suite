...
function Flash({ message }: { message: FlashMessage }) {
  ...
  if (!message.ok) return <ErrorBanner>{message.text}</ErrorBanner>
  return <p role="alert" className="rounded-xl border px-4 py-3 text-sm border-emerald-500/30 bg-emerald-500/10 text-emerald-600">{message.text}</p>
}
    ...
    <AppShell title="Ultrafine Invoice Booking Tracker">
      <div className="flex flex-col gap-6">
        ...
        <GlassCard padding="lg">
          ...
          {latest?.available && latest.rows?.length ? <div className="mt-4 table-shell">
