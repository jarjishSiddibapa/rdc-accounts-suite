...
export default function InvoiceBookingTracker() {
  ...
  async function saveSettings() {
    ...
    try {
      ...
      setSettings(next); setStatus(next); setPassword(''); setSenderPassword(''); setMailTo(recipientText(next.mail_to)); setMailCc(recipientText(next.mail_cc)); setMessage({ ok: true, text: 'Tracker automation settings saved.' })
    } catch (error) { setMessage({ ok: false, text: isAdmin && error instanceof ApiError ? error.message : PUBLIC_ISSUE_MESSAGE }) }
  ...
  async function uploadSession(file: File) {
    ...
    try { const form = new FormData(); form.append('file', file); const next = await postForm<Settings>(`${BASE}/session`, form); setSettings(next); setStatus(next); setMessage({ ok: true, text: 'Secure DMS browser session imported.' }) }
    catch (error) { setMessage({ ok: false, text: isAdmin && error instanceof ApiError ? error.message : PUBLIC_ISSUE_MESSAGE }) }
    finally { setBusy(null) }
  }
  ...
  async function testMail() {
    ...
    try { await post(`${BASE}/test-mail`, { subject_template: settings.subject_template, body_template: settings.body_template }); setMessage({ ok: true, text: `Test tracker mail sent to ${user?.email}.` }) }
    catch (error) { setMessage({ ok: false, text: isAdmin && error instanceof ApiError ? error.message : PUBLIC_ISSUE_MESSAGE }) }
    finally { setBusy(null) }
  }
  ...
  async function saveMapping() {
    ...
    try {
      ...
      setEditingId(null); setMappingDraft(emptyMapping); await loadMappings(); setMessage({ ok: true, text: 'Tracker mapping saved.' })
    } catch (error) { setMessage({ ok: false, text: isAdmin && error instanceof ApiError ? error.message : PUBLIC_ISSUE_MESSAGE }) }
    finally { setBusy(null) }
  }

  async function archiveOrRestore(row: MappingRow) {
    setBusy(`mapping-${row.id}`)
    try { if (row.is_deleted) await post(`${BASE}/mappings/${row.id}/restore`); else await del(`${BASE}/mappings/${row.id}`); await loadMappings() }
    catch (error) { setMessage({ ok: false, text: isAdmin && error instanceof ApiError ? error.message : PUBLIC_ISSUE_MESSAGE }) }
    finally { setBusy(null) }
  }
  ...
  return (
    <AppShell title="Ultrafine Invoice Booking Tracker">
      <div className="flex flex-col gap-6">
        ...
        {jobId && <ProgressPanel<CheckResult> jobId={jobId} cancelOnTabClose={false} poller={(id) => get<JobState<CheckResult>>(`${BASE}/jobs/${id}`)} onCancel={() => post(`${BASE}/jobs/${jobId}/cancel`)} onDone={(result) => { setJobId(null); setBusy(null); setMessage({ ok: true, text: result?.skipped ? result.message ?? 'A check is already running.' : `Checked ${formatIndianNumber(result?.total_pages_scanned ?? 0)} pages and found ${formatIndianNumber(result?.total_pending ?? 0)} pending invoices.` }); void Promise.all([loadSummary(), loadChecks(), loadNotifications()]) }} onError={(error) => { setJobId(null); setBusy(null); setMessage({ ok: false, text: isAdmin || error === ACCOUNT_IN_USE_MESSAGE ? error : PUBLIC_ISSUE_MESSAGE }); void Promise.all([loadSummary(), loadChecks(), loadNotifications()]) }} />}
        ...
        {isAdmin && settings && <>
          ...
          <GlassCard padding="lg" className="flex flex-col gap-5">
            <div className="flex items-center gap-3"><Mail className="h-5 w-5 text-accent" /><div><h2 className="font-display text-xl font-semibold text-ink">Daily tracker mail</h2><p className="text-sm text-ink-dim">A successful scheduled scan produces the table and an Excel attachment exactly once per date.</p></div></div>
            <div className="grid gap-4 lg:grid-cols-2">
          ...
          <GlassCard padding="lg" className="flex flex-col gap-4">
            <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><h2 className="font-display text-xl font-semibold text-ink">DMS tracker mappings</h2><p className="mt-1 text-sm text-ink-dim">Search, reorder, pause, edit, archive, or restore centrally managed work queues.</p></div><div className="relative"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-ink-faint" /><input className="field-control pl-9" placeholder="Search mappings" value={mappingSearch} onChange={(event) => { setMappingSearch(event.target.value); setMappingPage(1) }} /></div></div>
            ...
            <div className="overflow-x-auto"><table className="w-full min-w-[70rem] text-left text-sm"><thead><tr className="border-b border-border text-xs text-ink-faint"><th className="px-2 py-3">Order</th><th className="px-2 py-3">Location</th><th className="px-2 py-3">Responsible person</th><th className="px-2 py-3">DMS work queue</th><th className="px-2 py-3">Active</th><th className="px-2 py-3 text-right">Actions</th></tr></thead><tbody>{mappings.map((row) => <tr key={row.id} className="border-b border-border/60 align-top"><td className="px-2 py-3 tabular-nums text-ink-dim">{row.sort_order}</td><td className="px-2 py-3 font-semibold text-ink">{row.location}</td><td className="px-2 py-3 text-ink-dim">{row.responsible_person}</td><td className="max-w-lg px-2 py-3 text-xs leading-5 text-ink-dim"><span className="block">{row.queue_label}</span><span className="text-ink-faint">{row.queue_key}</span></td><td className="px-2 py-3">{row.is_active ? 'Yes' : 'Paused'}</td><td className="px-2 py-3 text-right"><button className="mr-2 inline-flex min-h-9 items-center gap-1 rounded-lg px-2 text-xs font-semibold text-accent hover:bg-accent/10" onClick={() => { setEditingId(row.id); setMappingDraft({ location: row.location, responsible_person: row.responsible_person, queue_label: row.queue_label, queue_key: row.queue_key, sort_order: row.sort_order, is_active: row.is_active }) }}><Pencil className="h-3.5 w-3.5" />Edit</button><button className="inline-flex min-h-9 items-center gap-1 rounded-lg px-2 text-xs font-semibold text-red-500 hover:bg-red-500/10" disabled={busy === `mapping-${row.id}`} onClick={() => void archiveOrRestore(row)}><Trash2 className="h-3.5 w-3.5" />{row.is_deleted ? 'Restore' : 'Archive'}</button></td></tr>)}</tbody></table>{mappings.length === 0 && <p className="py-8 text-center text-sm text-ink-faint">No mappings match this search.</p>}</div>
            ...
            {!showArchived && <div className="subpanel grid gap-3 p-4 lg:grid-cols-6"><Field label="Order"><input type="number" min={0} className="field-control" value={mappingDraft.sort_order} onChange={(event) => setMappingDraft({ ...mappingDraft, sort_order: Number(event.target.value) })} /></Field><Field label="Location"><input className="field-control" value={mappingDraft.location} onChange={(event) => setMappingDraft({ ...mappingDraft, location: event.target.value })} /></Field><Field label="Responsible person"><input className="field-control" value={mappingDraft.responsible_person} onChange={(event) => setMappingDraft({ ...mappingDraft, responsible_person: event.target.value })} /></Field><Field label="DMS work-queue label"><input className="field-control" value={mappingDraft.queue_label} onChange={(event) => setMappingDraft({ ...mappingDraft, queue_label: event.target.value })} /></Field><Field label="Queue key (fallback)"><input className="field-control" value={mappingDraft.queue_key} onChange={(event) => setMappingDraft({ ...mappingDraft, queue_key: event.target.value })} /></Field><div className="flex flex-col justify-end gap-2"><label className="flex items-center gap-2 text-sm text-ink-dim"><input type="checkbox" checked={mappingDraft.is_active} onChange={(event) => setMappingDraft({ ...mappingDraft, is_active: event.target.checked })} />Active</label><Button icon={editingId ? <Save className="h-4 w-4" /> : <Plus className="h-4 w-4" />} loading={busy === 'mapping'} disabled={!mappingDraft.location.trim() || !mappingDraft.responsible_person.trim() || !mappingDraft.queue_label.trim()} onClick={() => void saveMapping()}>{editingId ? 'Save mapping' : 'Add mapping'}</Button>{editingId && <button className="text-xs text-ink-dim" onClick={() => { setEditingId(null); setMappingDraft(emptyMapping) }}>Cancel edit</button>}</div></div>}
          </GlassCard>
        </>}
        ...
        <GlassCard padding="lg">
          ...
          <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[58rem] text-left text-sm"><thead><tr className="border-b border-border text-xs text-ink-faint"><th className="px-2 py-3">Created at (IST)</th><th className="px-2 py-3">Subject</th><th className="px-2 py-3">Attachment</th><th className="px-2 py-3">Status</th><th className="px-2 py-3">Delivery details</th></tr></thead><tbody>{notifications.map((row) => <tr key={row.id} className="border-b border-border/60 align-top"><td className="whitespace-nowrap px-2 py-3 text-ink-dim">{formatIndianDateTime(row.created_at)}</td><td className="max-w-sm px-2 py-3 text-ink">{row.subject}</td><td className="max-w-xs px-2 py-3 text-xs text-ink-dim">{row.attachment_filename}</td><td className="px-2 py-3"><Badge status={row.status} /></td><td className="max-w-md whitespace-pre-wrap break-words px-2 py-3 text-xs text-ink-dim">{row.error_message || (row.sent_at ? `Sent ${formatIndianDateTime(row.sent_at)}` : 'Awaiting delivery')}</td></tr>)}</tbody></table>{notifications.length === 0 && <p className="py-8 text-center text-sm text-ink-faint">No scheduled tracker mails have been generated yet.</p>}</div>
