...
def _job_send(user_id: int, customers: list, progress_cb=None) -> dict:
    ...
    try:
        report = processing.send_bulk_mails(
...
async def preview(
    ...
    # Grouped into one shared "payment-reminder-<uuid>" directory (rather
    # than loose SCRATCH_DIR files) so the scratch-cleanup sweep can give
    # this preview-then-confirm-send workflow its own generous grace period
    ...
    job_dir = SCRATCH_DIR / f"payment-reminder-{uuid.uuid4()}"
    job_dir.mkdir(parents=True, exist_ok=True)

    data_path = str(await _save_upload(data_file, job_dir))

    emails_path = None
    if emails_file is not None and emails_file.filename:
        emails_path = str(await _save_upload(emails_file, job_dir))
