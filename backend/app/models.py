...
# NOTE: the IT POs Lookup application was removed (2026-09-18). Its two
# tables, po_lookup_statement_uploads and po_lookup_bank_transactions - an
# append-only audit trail of every bank statement Accounts had uploaded, plus
# every parsed transaction line from them - and the now-orphaned
# users.po_lookup_role column were deliberately left in the live database
# rather than dropped, since dropping them is destructive and irreversible.
# See deployment/mysql/20260918_remove_it_po_lookup.sql for the optional
# migration to actually drop them once that data is confirmed no longer
# needed.
